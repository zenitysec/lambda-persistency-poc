...
# Send an invocation result for the invoke that switched the runtime
def twist_respond_to_payload_invoke(lambda_runtime_client, invoke_id):
    response = {
...
def twist_main(invoke_id):
    ...
    if not twist_respond_to_payload_invoke(lambda_runtime_client, invoke_id):
        return
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("[!] Usage: <twist-runtime> <invoke-id>")
        sys.exit(1)
    ...
    print("[+] twist-runtime starting, invoke-id: " + invoke_id)
    twist_main(invoke_id)
